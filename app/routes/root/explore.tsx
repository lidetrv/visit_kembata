...
// Destination data for the Explore page
const allDestinations = [
  {
    name: "Hambericho Peak",
    text: "Trekking high above the clouds.",
    tag: "Mountains",
    img: "/assets/icons/Hambericho777.jpg",
  },
  {
    name: "Ajora Camping",
    text: "Lakeside views and cozy nights.",
    tag: "Camping",
    img: "/assets/icons/AjoraCamping.jpg",
  },
  {
    name: "Soso Waterfalls",
    text: "Discover hidden rainforest treasures.",
    tag: "Waterfalls",
    img: "/assets/icons/SosoWaterfalls.jpg",
  },
  {
    name: "Sunrise Plateau",
    text: "Panoramic views at dawn.",
    tag: "Mountains",
    img: "/assets/images/sunshinelove.jpg",
  },
  {
    name: "River Trek",
    text: "A challenging but rewarding trek.",
    tag: "Hiking",
    img: "/assets/images/waterfall.jpg",
  },
  {
    name: "The Sunset Group",
    text: "Unforgettable evening photography.",
    tag: "Camping",
    img: "/assets/images/sunsetgroup.jpg",
  },
  {
    name: "Forest Retreat",
    text: "Deep woods and tranquility.",
    tag: "Forests",
    img: "https://placehold.co/400x300/65A30D/ffffff?text=Forest+Trail",
  },
  {
    name: "Canyon Descent",
    text: "Explore the deep gorges.",
    tag: "Canyons",
    img: "https://placehold.co/400x300/CA8A04/ffffff?text=Canyon+View",
  },
];
...
export default function Explore() {
  ...
  const filteredDestinations = allDestinations.filter(
    (dest) => filter === "All" || dest.tag === filter
  );

  const tags = [
    { label: "All", icon: FaTag },
    { label: "Mountains", icon: FaMountain },
    { label: "Waterfalls", icon: FaWater },
    { label: "Camping", icon: FaTree },
    { label: "Hiking", icon: FaTree },
    { label: "Forests", icon: FaTree },
  ];
  ...
  return (
    <div className="font-sans text-gray-800">
      ...
      {/* Tag Area (Filters) */}
      <section className="py-8 bg-gray-100 px-6 md:px-16 border-b border-gray-200">
        <div className="flex flex-wrap justify-center gap-3">
          {tags.map((tag) => (
            <button
              key={tag.label}
              onClick={() => setFilter(tag.label)}
              className={`flex items-center space-x-2 px-5 py-2 rounded-full font-semibold transition-all shadow-md
                                ${
                                  filter === tag.label
                                    ? "bg-green-700 text-white shadow-green-400/50"
      ...
      {/* Photo Gallery / Destination Grid */}
      <section className="py-12 px-6 md:px-16">
        <h3 className="text-3xl font-bold text-center mb-8 text-green-700">
          {filter === "All" ? "All Destinations" : `${filter} Adventures`}
        </h3>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {filteredDestinations.map((dest, index) => (
            <motion.div
              ...
              whileHover={{
                y: -5,
                boxShadow: "0 10px 15px rgba(52, 211, 163, 0.3)",
              }}
            ...
            >
              <img
                ...
                onError={(e) => {
                  ...
                  target.onerror = null;
                  target.src =
                    "https://placehold.co/400x300/34D399/ffffff?text=Image+Loading";
                }}
              />
              <div className="p-4">
                <span className="inline-block text-xs font-medium text-white bg-green-500 rounded-full px-3 py-1 mb-2">
                  {dest.tag}
                </span>
                <h4 className="text-xl font-semibold text-gray-900 mb-2">
                  {dest.name}
                </h4>
                <p className="text-sm text-gray-600">{dest.text}</p>
              </div>
            </motion.div>
          ))}
        </div>
        {filteredDestinations.length === 0 && (
          <p className="text-center text-xl text-gray-500 py-10">
            No destinations found for this filter. Try another tag!
          </p>
        )}
      </section>
