...
interface Props {
  ...
  fieldChange: (value: string) => void;
  editorRef: ForwardedRef<MDXEditorMethods> | null;
  onImageUploaded?: (url: string) => void; // optional callback
}

/**
 * Appwrite client + storage
 */
 ...
 */
const imageUploadHandler = async (file: File, onImageUploaded?: (url: string) => void): Promise<string> => {
  try {
    // immediate preview
...
const Editor = ({ value, editorRef, fieldChange, onImageUploaded, ...props }: Props) => {
  return (
    <div>
      <MDXEditor
        markdown={value}
        ...
        plugins={[
          ...
          tablePlugin(),
          imagePlugin({
            imageUploadHandler: (file: File) => imageUploadHandler(file, onImageUploaded),
          }),
          thematicBreakPlugin(),
          markdownShortcutPlugin(),
          codeBlockPlugin({ defaultCodeBlockLanguage: "" }),
